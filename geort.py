...
#openai.api_key = 'sk-P08-f_f6FbNy1rISbkWBcA7WL1z8HG7zsHwnvcffAYT3BlbkFJenHOeqd8WvssZnpDg7xx4exqpoYrRI3QWlFCDuUGEA'  # Reemplaza con 

import pandas as pd
import random
import time  # Importar el módulo time para medir el tiempo
import csv   # Importar csv para guardar las respuestas
import os  # Importar os para comprobar la existencia de archivos
...
# Definir colores como constantes para facilitar su uso y mantenimiento
BUTTON_COLOR = (0.0, 0.8, 0.0, 1)  # Verde brillante (#00CC00)

# Preguntas predeterminadas para cada tema (se recomienda utilizar los archivos CSV proporcionados)
...
# Función para cargar preguntas desde CSV o usar preguntas predeterminadas
def cargar_preguntas(archivo_csv, default_questions):
    ...
    if os.path.isfile(archivo_csv):
        try:
            ...
            for _, row in df.iterrows():
                preguntas.append({
                    ...
                    'options': [str(row['Opcion_A']), str(row['Opcion_B']),
                                str(row['Opcion_C']), str(row['Opcion_D'])],
        ...
        print(f"Archivo {archivo_csv} no encontrado. Usando preguntas predeterminadas.")
        preguntas = default_questions
    return preguntas

# Cargar las preguntas de los archivos CSV o usar las predeterminadas
# Asegúrate de actualizar las rutas de los archivos CSV según tu sistema
independencia_preguntas = cargar_preguntas(r'C:/Users/USER/OneDrive/Desktop/Geort/preguntas idependencia.csv',
                                           default_independencia_preguntas)
boyaca_preguntas = cargar_preguntas(r'C:/Users/USER/OneDrive/Desktop/Geort/preguntas bataya.csv',
                                    default_boyaca_preguntas)
guerra_preguntas = cargar_preguntas(r'C:/Users/USER/OneDrive/Desktop/Geort/preguntas guerra mil.csv',
                                    default_guerra_preguntas)
...
class RoundedButton(Button):
    ...
    def __init__(self, **kwargs):
        ...
        self.color = kwargs.get('color', (1, 1, 1, 1))  # Texto blanco
